/*
 ...
 EXCLUSIVE. The function takes 2 values (LowerValue and UpperValue), it shall determine the values in between, and then
...
unsigned char * arrayInBetweenUpperAndLower(int lowerValue,int upperValue, unsigned int * sizeOfArray){
	...
	*sizeOfArray = upperValue - lowerValue - 1;
	...
	for(i = 0, j = upperValue - 1; i < *sizeOfArray; i++, j--){
		numbers[i] = j;
	}
